...
class ActionSpace:
    ...
    def load_bitmap(self, bitmap):
        bits = [0] * 46
        for i in range(46 - 1, -1, -1):
            bits[i] = bitmap & 1
            bitmap >>= 1

        self.allowed_tiles_discard = bits[0:37]
        self.allowed_riichi = bits[37]
        self.allowed_chii_types = bits[38:41]
        self.allowed_pon = bits[41]
        self.allowed_kan = bits[42]
        self.allowed_win = bits[43]
        self.allowed_draw = bits[44]
    ...
    while game_data.current_round is not None:
        while ev := game_data.get_next_event():
            if isinstance(ev, tenhou_decoder.DrawTileEvent):
                ...
                # We don't care about state before hand so just process the event
                ...
                action_space.allowed_draw = (not game_data.current_round.did_someone_call() and
                            ...
                            map(lambda t: (t.tile % 9 == 0 or t.tile % 9 == 8) or t.tile >= 7,
                ...
                if curr_player.is_rii:
                    ...
                    # Check either: (ankan) or (added kan)
                    # action_space.allowed_kan = (any(
                    #     i.meld_type == 2 and i.tiles[0].tile == ev.tile.tile for i in curr_player.melds)
                    #                             or )

                    can_add_kan = any(i.meld_type == 2 and t34[i.tiles[0].tile] > 0 for i in curr_player.melds)
                    can_ankan = any(i == 4 for i in t34)

                    action_space.allowed_kan = (
                                # Added Kan
                ...
                action_space.allowed_riichi = game_data.scores[ev.player] >= 10 and get_rii_tiles(t34, True)

                match type(ne := game_data.get_next_event()):
                    case tenhou_decoder.RiichiEvent:
                        assert action_space.allowed_riichi
                        correct_decision = 37

                        yield game_data.dump_compressed(ev.player), action_space.dump_bitmap(), correct_decision, 0
                        game_data.process_event()
                        assert isinstance(game_data.get_next_event(), tenhou_decoder.DiscardTileEvent)
                    case tenhou_decoder.TsumoEvent:
                        # assert action_space.allowed_win
                        action_space.allowed_win = True
                        correct_decision = 43
                        yield game_data.dump_compressed(ev.player), action_space.dump_bitmap(), correct_decision, 0
                    case tenhou_decoder.CallTileEvent:
                        assert action_space.allowed_kan
                        correct_decision = 42
                        yield game_data.dump_compressed(ev.player), action_space.dump_bitmap(), correct_decision, \
                        ne.meld.tiles[0].tile

                        game_data.process_event()
                    case tenhou_decoder.RyuuyokuEvent:
                        if action_space.allowed_draw:
                            correct_decision = 44
                            yield game_data.dump_compressed(ev.player), action_space.dump_bitmap(), correct_decision, 0
                    case tenhou_decoder.DiscardTileEvent:
                        if action_space.allowed_draw or action_space.allowed_kan or action_space.allowed_win or action_space.allowed_riichi:
                            correct_decision = 45
                        else:
                            correct_decision = 0
                        yield game_data.dump_compressed(
                            ev.player), action_space.dump_bitmap(), correct_decision, ne.tile.tile
                    case _:
                        raise RuntimeError(f"Event {ev} -> {ne} should not happen")
            elif isinstance(ev, tenhou_decoder.DiscardTileEvent):
                # Pre Discard: Decide what tile to discard
                action_space = ActionSpace()

                yield game_data.dump_compressed(ev.player), action_space.dump_bitmap(), 0, ev.tile.tile
                game_data.process_event()
