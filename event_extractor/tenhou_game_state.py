import event_extractor.tenhou_decoder as tenhou_decoder
import shanten_calcs


def to_tile_34(tiles: list[tenhou_decoder.Tile]):
    t = [0 for _ in range(34)]

    for i in tiles:
        t[i.tile] += 1 + 0.5 * i.is_aka()

    return t


def pad_0(arr: list, final_len: int):
    return arr + [0 for _ in range(final_len - len(arr))]


class DiscardedTile(tenhou_decoder.JsonSerializable):
    def __init__(self, tile: tenhou_decoder.Tile, discard_type: int, riichi_tile: bool):
        self.tile = tile
        self.discard_type = discard_type  # 0 = tedashi, 1 = tsumogiri
        self.riichi_tile = riichi_tile
        self.called = False


class Meld(tenhou_decoder.JsonSerializable):
    def __init__(self, meld_type: int, tiles: list[tenhou_decoder.Tile]):
        self.meld_type = meld_type  # 1 = chi, 2 = pon, 3 = kan
        self.tiles = tiles
...
class PlayerState(tenhou_decoder.JsonSerializable):
    ...
    def discard_tile(self, tile: tenhou_decoder.Tile):
        ...
        tsmugori = int(idx == len(self.closed_hand))
        self.discard_pool.append(DiscardedTile(tile, tsmugori, self._last_action == "riichi"))
        self.closed_hand.sort()
        if self._last_action == "riichi":
            self._last_action = "riichi_discard"
        elif "kan" in self._last_action:
            self._last_action = self._last_action + "_discard"
        else:
    ...
    def break_ippatsu(self):
        if self._last_action == "riichi_discard":
            self._last_action = "discard"
...
class GameState:
    ...
    def dump_compressed(self, player_pov: int):
        ...
        # Metadata
        arr[0] = self.current_round.round_no // 4  # Round Wind
        arr[1] = self.current_round.round_no % 4  # Dealer
        arr[2] = player_pov  # Which player is doing the action
        arr[3] = self.current_round.rii_sticks  # Riichi Sticks in Pot
        arr[4] = self.current_round.honba_count  # Honba Count
        arr[5] = self.current_round.tiles_left  # Tiles left in the wall
        arr[6] = self.scores[0]  # p0 score
        arr[7] = self.scores[1]  # p1 score
        arr[8] = self.scores[2]  # p2 score
        arr[9] = self.scores[3]  # p3 score
        arr[10] = int(self.current_round.players[0].is_rii)  # p0 rii
        arr[11] = int(self.current_round.players[1].is_rii)  # p1 rii
        arr[12] = int(self.current_round.players[2].is_rii)  # p2 rii
        arr[13] = int(self.current_round.players[3].is_rii)  # p3 rii

        # Dora Indicators
        arr += pad_0([i.i for i in self.current_round.dora_indicators], 14)

        # Player Data
        for i in range(4):
            # Closed Hand
            arr += pad_0([j.i for j in self.current_round.players[i].closed_hand], 14)

            # Melds
            temp = []
            for k in self.current_round.players[i].melds:
                temp.append(k.meld_type)
                temp += pad_0([m.i for m in k.tiles], 4)
...
def test_gen_dump():
    ...
    game_data = tenhou_decoder.extract_bz2(h)
